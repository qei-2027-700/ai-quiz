...
function extractSqlString(src: string, startAt: number): { value: string; end: number } {
  let i = startAt;
  // 開始クォートを探す
...
function parseSeederSql(sql: string): { topicId: string; questions: ParsedQuestion[] } {
  const questions: ParsedQuestion[] = [];
  const choicesByQuestion: Record<string, ParsedChoice[]> = {};
  const explanationByQuestion: Record<string, string> = {};
  let topicId = "";
  ...
  while ((m = qRegex.exec(sql)) !== null) {
    ...
    questions.push({
      ...
      difficulty: parseInt(diffMatch[1]) as 1 | 2 | 3,
      genre: unescapeSql(genreR.value) as ParsedQuestion["genre"],
      choices: [],
      explanation: "",
    });
  }
...
function generateMockTs(topicId: string, questions: ParsedQuestion[]): string {
  const lines: string[] = [];
  ...
  for (const q of questions) {
    ...
    for (const c of q.choices) {
      lines.push(
        `      { id: '${c.id}', text: ${JSON.stringify(c.text)}, isCorrect: ${c.isCorrect}, sortOrder: ${c.sortOrder} },`
      );
    }
    lines.push("    ],");
